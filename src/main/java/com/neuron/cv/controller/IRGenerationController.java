...
import com.neuron.cv.service.IRManagerService;
import lombok.extern.slf4j.Slf4j;

@RestController
@RequestMapping("/api/v1/report")
@Slf4j
public class IRGenerationController {
	
	@Autowired
	private IRManagerService irManagerService;
	...
	public ResponseEntity<String> generateInspectionReport(@RequestHeader("hash-key") String hashKey,
		...
		try {
			...
			if (hashKey.equals(generatedKey))
				return new ResponseEntity<String>(irManagerService.generateInspectionReport(paramDTO, true), HttpStatus.OK);
